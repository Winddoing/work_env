#!/bin/bash
##########################################################
# File Name		: upgrade_latest_kernel.sh
# Author		: winddoing
# Created Time	: 2019年10月30日 星期三 13时55分43秒
# Description	: auto upgrade latest kernel
#       1. ubuntu18.04 test OK
##########################################################
...
get_latest_version() {
    ...
    for i in ${latest_version[@]}; do
        if version_ge $i $kernel_version; then
            kernel_arr+=($i);
            kid=$((kid + 1))
        fi
    done
